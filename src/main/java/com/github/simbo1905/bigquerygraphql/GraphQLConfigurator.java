...
public class GraphQLConfigurator {
    ...
    @ConfigProperty(name = "gcp.bigtable-table")
    String table;
    ...
    protected RuntimeWiring.Builder wire(RuntimeWiring.Builder wiring, List<FieldMetaData> mappings) {
        for (FieldMetaData fieldMetaData : mappings) {
            ...
            final Set<String> qualifiers =
                    Arrays.stream(fieldMetaData.qualifiesCsv.split(",")).collect(Collectors.toSet());
            wiring = wiring
                    .type(fieldMetaData.typeName, builder -> {
                        // a blocking data fetcher
                        DataFetcher blockingDataFetcher = bigtableRunner.queryForOne(
                                table,
                                fieldMetaData.family,
                                qualifiers,
